...
* `test-5090-fp8-reduction-width`, `test-5090-fp6-reduction-width`, and `test-5090-fp4-reduction-width`, for probing RTX 5090 low-precision tensor-core reduction width with direct PTX `mma.sync.aligned` instructions. The matching `test-5090-fp8-reduction-repeat`, `test-5090-fp6-reduction-repeat`, and `test-5090-fp4-reduction-repeat` targets run a chained-MMA cross-check. These targets use `compute_120a`/`sm_120a` because FP6/FP4 `.kind::f8f6f4` PTX requires the SM120 accelerated feature target.
